...
$type_id = isset($_GET['type_id']) ? sanitizeInput($_GET['type_id']) : '';
...
// Base SQL query
$sql = "SELECT po.*, p.product_name, p.product_code, ot.type_name, u.username 
        FROM product_outgoing po
        JOIN products p ON po.product_id = p.product_id
        JOIN outgoing_types ot ON po.type_id = ot.type_id
        JOIN users u ON po.user_id = u.user_id
        WHERE DATE(po.transaction_date) BETWEEN ? AND ?";

// Add search condition
if (!empty($search)) {
    $search_term = "%$search%";
    $sql .= " AND (p.product_code LIKE ? OR p.product_name LIKE ? OR po.destination LIKE ? OR po.notes LIKE ?)";
}
...
$total_outgoing = $total_result->fetch_assoc()['total'] ?? 0;
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <form method="get" class="row g-3 align-items-end mb-4">
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control" id="search" name="search"
                                        placeholder="Cari kode/nama barang, tujuan atau catatan..."
                                        value="<?php echo htmlspecialchars($search); ?>">
                                    <?php if (!empty($search) || !empty($product_id) || !empty($type_id) || !empty($user_id)): ?>
                            ...
                            <div class="col-md-4 d-flex gap-2">
                                ...
                                <a href="outgoing.php" class="btn btn-warning w-100">reset</a>
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <h6>Total Barang Keluar: <span class="badge bg-success"><?php echo number_format($total_outgoing, 0); ?> item</span></h6>
                        </div>
                        <div class="mb-3">
                            <!-- Active Filters Summary -->
                            <?php if (!empty($product_id) || !empty($type_id) || !empty($user_id)): ?>
